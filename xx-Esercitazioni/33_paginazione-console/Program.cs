...
static void MostraProdottiPaginati(List<Prodotto> prodotti,int articoliPerPagina)
{
    ...
    while(true)
    {
        ...
        Console.WriteLine(new string('-',40)); // Linea divisoria 

        var prodottiPagina = prodotti.Skip((paginaCorrente -1)*articoliPerPagina).Take(articoliPerPagina);//Lista di prodotti in modo da poter fare la paginazione
        //paginazione                //Salta i prodotti delle pagine precendenti  //Prendi i prodotti della pagina corrente
        ...
        Console.WriteLine("Navigazione: [<--] Pagina precedente | [-->] Prossima pagina | [Esc] Esci\n");
        //uso il readkey in modo da non dover premere invio

        var input = Console.ReadKey(true).Key; // il true serve per non mostrare il tasto premuto

        if(input == ConsoleKey.RightArrow && paginaCorrente < totalePagine)
        {
            paginaCorrente++; // va avanti avendo come limite il totale della pagine
        }
